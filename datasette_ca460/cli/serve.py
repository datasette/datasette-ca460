...
@click.option("-p", "--port", type=int, default=8007, help="Port to serve on")
@click.option("--host", default="127.0.0.1", help="Host to serve on")
@click.option("--no-open", is_flag=True, help="Don't open browser automatically")
def ca460_serve(db_path, port, host, no_open):
    "Start Datasette serving a CA 460 database"
    import uvicorn
    ...
    if not no_open:
        url = f"http://{host}:{port}/{db_name}/-/ca460"
